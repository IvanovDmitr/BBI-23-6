...
namespace ЛР_10_1
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (var tournament in tournaments)
            {
                tournament.Teams = tournament.Teams.OrderByDescending(t => t.CalculatePoints()).ToList();
            }
            ...
            {
                var sortedTeams = t.Teams.OrderByDescending(team => team.CalculatePoints()).ToList();
                var newTournament = new TournamentTable(t.Name);
                newTournament.Teams.AddRange(sortedTeams.Take(sortedTeams.Count / 2));
